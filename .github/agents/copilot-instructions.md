...
- TypeScript 5 + React 18 (frontend only; no backend change) + React, Vite, Vitest, Testing Library — no new dependencies introduced (036-rename-practice-train)
...
- TypeScript (React 18, Vite 7), Rust 1.93 (WASM), GitHub Actions + `vite-plugin-pwa` (v1.x), Workbox 7, wasm-pack; hosting TBD (Cloudflare Pages / Netlify / GitHub Pages); analytics TBD (Umami Cloud / Plausible) (001-pwa-hosting-service)
- None (analytics data stored in SaaS provider; no new local storage) (001-pwa-hosting-service)
- Rust 2024 edition (backend/WASM), TypeScript strict (frontend) + quick-xml 0.31 (MusicXML parsing), serde 1.0 with `default` feature (JSON serialization/deserialization), wasm-bindgen 0.2 (WASM bindings), Tone.js (frontend audio scheduling), Vitest (frontend unit tests) (047-repeat-volta-playback)
...
- Rust 1.93.0 (stable) + `serde_json` (JSON parsing), `serde` (Serialize/Deserialize) — no new dependencies introduced (052-refactor-layout-engine)
- N/A (pure computation module) (052-refactor-layout-engine)
- Rust (latest stable), TypeScript 5 / React 18 + wasm-bindgen, wasm-pack, Vite, SMuFL Unicode glyphs (U+E260–U+E264) (001-fix-nocturne-layout)
...
- TypeScript (strict mode), React 18+ + Vitest (testing), @testing-library/react, IndexedDB (via plugin-api) (087-one-goal-40pct-cap)
- IndexedDB `goals` + `sessions` stores; localStorage index layer (087-one-goal-40pct-cap)
...
## Recent Changes
- 087-one-goal-40pct-cap: Added TypeScript (strict mode), React 18+ + Vitest (testing), @testing-library/react, IndexedDB (via plugin-api)
