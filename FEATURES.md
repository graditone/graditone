...
- **Train** built-in plugin (v2) - Piano training exercise with scoring
  ...
  - **Measure-boundary chord validation** (Feature 098) - A long chord (e.g., a whole-note chord filling a 4/4 measure) is validated as correct the moment its full notated duration is held, including when it is released at the next measure's downbeat while the following chord is pressed; a release that has reached the required hold is never recorded as "released too early", while genuine early releases remain detected and penalised
