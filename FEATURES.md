...
- **Train** built-in plugin (v2) - Piano training exercise with scoring
  ...
  - **Measure-boundary chord validation** (Feature 098) - A long chord (e.g., a whole-note chord filling a 4/4 measure) is validated as correct the moment its required hold is reached, including when it is released at the next measure's downbeat while the following chord is pressed; releases above the threshold are never "released too early", while genuine early releases remain detected and penalised. A modest release margin of up to 20% of the duration (capped at 1500 ms) lets players begin repositioning their fingers for the next chord without losing a correct validation
