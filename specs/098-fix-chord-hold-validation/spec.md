...
- What happens if the learner changes the chord slightly before the downbeat (a few tens of ms)? Acceptance follows the existing early-acceptance window; only holds that genuinely fall short of the threshold are early-release.
...
(`isHoldAccepted(requiredHoldMs, elapsedMs)` — ≥90% of the required duration,
capped at 500 ms early) is applied at every decision point: the MIDI release
...
- The chord's notated duration for a whole note filling a 4/4 measure equals the measure at the current tempo; the required hold must equal that duration (subject only to the existing early-acceptance window).
- The supported tempo range is 10–300 BPM, consistent with specs 085/086.
- The existing early-acceptance window (90% of duration, capped at 500 ms early) remains in force; the fix adjusts validation of the boundary/ordering, not the tolerance itself.
